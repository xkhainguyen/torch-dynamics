...
void _dynamics(const double* q_in, const double* qdot_in, const double* tau_in, double* qddot_out) {
...
void _derivatives(const double* q_in, const double* qdot_in, const double* tau_in, 
...
void dynamics_kernel(const scalar_t* q_in_ptr, const scalar_t* qdot_in_ptr, const scalar_t* tau_in_ptr, scalar_t* qddot_out_ptr, int q_size, int batch_size) {
    at::parallel_for(0, batch_size, 1, [&](int64_t start, int64_t end) {
        for (int64_t b=start; b<end; b++) {
            _dynamics(q_in_ptr + b*q_size, qdot_in_ptr + b*q_size, tau_in_ptr + b*q_size, qddot_out_ptr + b*q_size);
        }
    });
}

template <typename scalar_t>
void derivatives_kernel(const scalar_t* q_in_ptr, const scalar_t* qdot_in_ptr, const scalar_t* tau_in_ptr, scalar_t* qddot_jac_qout_ptr, scalar_t* qddot_jac_qdotout_ptr, scalar_t* qddot_jac_tauout_ptr, int q_size, int batch_size) {
    at::parallel_for(0, batch_size, 1, [&](int64_t start, int64_t end) {
        for (int64_t b=start; b<end; b++) {
            _derivatives(q_in_ptr + b*q_size, qdot_in_ptr + b*q_size, tau_in_ptr + b*q_size, qddot_jac_qout_ptr + b*q_size*q_size, qddot_jac_qdotout_ptr + b*q_size*q_size, qddot_jac_tauout_ptr + b*q_size*q_size);
        }
    });
}
